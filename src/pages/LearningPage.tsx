...
useEffect(() => {
  ...
  useEffect(() => {
    // We check if the quiz is done, the user is logged in, and lesson exists
  ...
  useEffect(() => {
    ...
    const fetchLesson = async () => {
      ...
      try {
        ...
        if (!API_KEY) {
          throw new Error("⚠️ VITE_GEMINI_API_KEY is not configured in your .env file");
        }
        ...
        if (err.message?.includes("VITE_GEMINI_API_KEY")) {
          ...
          errorDetails = "Check your .env file";
        } else if (err.message?.includes("API_KEY_INVALID") || err.status === 400) {
          errorMessage = "Invalid API Key";
          errorDetails = "Your Gemini API key is invalid. Generate a new one at ai.google.dev";
        } else if (err.status === 403) {
          errorMessage = "API Key Restricted";
          errorDetails = "Check API key restrictions in Google Cloud Console";
        } else if (err.status === 429) {
          errorMessage = "Rate Limit Exceeded";
          errorDetails = "Too many requests. Wait a moment and try again";
        } else if (err.message?.includes("fetch")) {
          errorMessage = "Network Error";
          errorDetails = "Check your internet connection";
        } else if (err instanceof SyntaxError) {
          errorMessage = "JSON Parse Error";
          errorDetails = "The AI returned invalid data";
        } else {
          errorMessage = err.message || "Failed to load lesson";
          errorDetails = err.status ? `Status Code: ${err.status}` : "";
        }
    ...
    <section className="py-8 bg-muted/30 min-h-screen flex flex-col relative pb-24">
      <div className="container mx-auto px-4 max-w-3xl flex-grow flex flex-col">
        <div className="mb-4 flex justify-between items-center">
          ...
          {!isLoading && !quizCompleted && lesson && <div className="flex items-center gap-4"><div className="text-sm font-bold text-primary bg-primary/10 px-3 py-1 rounded-full">Score: {Math.round((score / (currentQuestionIndex + 1)) * 100)}%</div><div className="text-sm font-medium text-muted-foreground">Q: {currentQuestionIndex + 1}/{lesson.questions.length}</div></div>}
        </div>
        ...
        <Card className={`border-primary/20 shadow-lg flex flex-col flex-grow overflow-hidden min-h-[600px] transition-all duration-300`}>
          <CardHeader className="bg-primary/5 border-b py-4 flex flex-row items-center justify-between">
            ...
            {!isLoading && !quizCompleted && <Button variant="ghost" size="icon" onClick={handleSpeak} className={isSpeaking ? "text-red-500 animate-pulse" : "text-primary"}>{isSpeaking ? <StopCircle className="w-6 h-6" /> : <Volume2 className="w-6 h-6" />}</Button>}
          </CardHeader>
          ...
          {isLoading && (
              <div className="flex flex-col items-center justify-center flex-grow h-full text-center p-8 animate-in fade-in duration-500">
                <h2 className="text-3xl font-extrabold text-primary mb-6 animate-pulse">
                  Hold On while we are loading your lesson Content
                </h2>
                ...
                <div className="grid gap-6 max-w-md w-full">
                    <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl shadow-sm">
                       ...
                       <p className="text-blue-900 font-medium italic text-base">
  "{FUN_FACTS[funFactIndex]}"
            ...
            {lesson && !isLoading && !quizCompleted && (
               <div className="flex flex-col h-full">
                 <div className="bg-blue-50 p-5 rounded-xl text-blue-900 mb-8 border border-blue-100 shadow-sm"><div className="flex items-center gap-2 mb-2"><BookOpen className="w-5 h-5 text-blue-600" /><h4 className="font-bold text-sm uppercase tracking-wide text-blue-700">Concept Overview</h4></div><p className="text-base leading-relaxed opacity-90">{lesson.topicIntro}</p></div>
                 <div className="mb-6">
                 ...
                 <div className="mt-auto pt-4 border-t">
                     ...
                     ) : !isCorrectState && <Button className="w-full py-6 text-lg" onClick={handleNextQuestion} variant="secondary">Got it</Button>}
                 </div>
               </div>
            )}
            ...
            {quizCompleted && lesson && (
                <div className="flex flex-col items-center justify-center flex-grow text-center animate-in zoom-in-95">
                    ...
                    <h2 className="text-4xl font-bold mb-2 text-primary">Fantastic!</h2><p className="text-muted-foreground mb-8 text-lg">You have mastered this skill.</p>
                    <div className="w-full max-w-md bg-card border p-8 rounded-xl mb-8 shadow-sm"><div className="text-7xl font-black text-primary mb-2 tracking-tighter">{Math.round((score / lesson.questions.length) * 100)}</div><div className="text-sm text-muted-foreground uppercase tracking-wide font-semibold">SmartScore</div></div>
                    <div className="flex gap-4 w-full max-w-md"><Button variant="outline" className="flex-1 h-12" asChild><Link to={backUrl}>Back to Skills</Link></Button><Button className="flex-1 h-12" onClick={handleRetry}><RefreshCcw className="w-4 h-4 mr-2" /> Practice Again</Button></div>
                </div>
            )}
          </CardContent>
        </Card>
        ...
        {isChatOpen && (
          <div className="bg-white w-80 h-96 rounded-2xl shadow-2xl border border-gray-200 flex flex-col mb-4 overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-300">
            ...
            <div className="flex-grow p-4 overflow-y-auto bg-gray-50 space-y-3">
                ...
                {isChatLoading && <div className="flex items-start"><div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 mr-2"><Bot className="w-4 h-4" /></div><div className="bg-white p-3 rounded-2xl rounded-tl-none shadow-sm border border-gray-100 text-sm text-gray-500 flex items-center gap-2"><Loader2 className="w-3 h-3 animate-spin" /> Thinking...</div></div>}
                <div ref={chatEndRef} />
            </div>
